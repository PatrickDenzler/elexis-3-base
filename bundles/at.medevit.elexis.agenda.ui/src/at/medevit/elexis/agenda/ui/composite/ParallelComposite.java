...
public class ParallelComposite extends Composite implements ISelectionProvider, IAgendaComposite {
	...
	public ParallelComposite(MPart part, ESelectionService selectionService,
		...
		loadEventsFunction =
			new LoadEventsFunction(browser, "loadEventsFunction", scriptingHelper, uiSynchronize); //$NON-NLS-1$
		...
		browser.addProgressListener(new ProgressAdapter() {
			...
			public void completed(ProgressEvent event){
				String dayStartsAt =
					ConfigServiceHolder.get().get("agenda/beginnStundeTagesdarstellung", "0000"); //$NON-NLS-1$ //$NON-NLS-2$
				String dayEndsAt =
					ConfigServiceHolder.get().get("agenda/endStundeTagesdarstellung", "2359"); //$NON-NLS-1$ //$NON-NLS-2$
