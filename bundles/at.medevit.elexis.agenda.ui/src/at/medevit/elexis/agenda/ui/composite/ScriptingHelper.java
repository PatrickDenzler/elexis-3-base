...
public class ScriptingHelper {
	...
	private String parseTime(String time){
		if (time.length() < 6 || time.lastIndexOf(":") != 5) { //$NON-NLS-1$
			...
			time = time.replaceAll(":", ""); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void scrollToNow(){
		if (doScroll) {
			String script =
				"var now = $('#calendar').fullCalendar('getNow'); if (now >= $('#calendar').fullCalendar('getView').intervalStart && now < $('#calendar').fullCalendar('getView').intervalEnd){ setTimeout( function(){$('.fc-scroller').scrollTop($('.fc-now-indicator').position().top - ($('#calendar').height() / 2) );}  , 500 );}"; //$NON-NLS-1$
			SingleSourceUtil.executeScript(browser, script);
		}
	}
