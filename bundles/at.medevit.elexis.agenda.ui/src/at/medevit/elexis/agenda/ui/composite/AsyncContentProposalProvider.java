...
public abstract class AsyncContentProposalProvider<T extends Identifiable>
			...
			CompletableFuture.runAsync(() -> {
				...
				while (startMonitoring) {
					...
					if (contents.hashCode() != lastQueriedContentHash) {
						...
						String[] searchParts = contents.toLowerCase().split(" "); //$NON-NLS-1$
	...
	public static String getElexisDateSearchString(String value){
		...
		String ret = value.replaceAll("%", ""); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (ret.matches("[0-9]{3,}")) { //$NON-NLS-1$
			...
			// as in 01.02.1932
			int dotCount = ret.length() - ret.replace(".", "").length(); //$NON-NLS-1$ //$NON-NLS-2$
